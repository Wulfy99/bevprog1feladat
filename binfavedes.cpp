...
main (int argc, char *argv[])
{
    ...
    while (beFile.read ((char *) &b, sizeof (unsigned char)))
    {
        ...
        if (b == 0x3e) // 62 hexadecimálisan írva.| Binarisan: 111110 | \0x3e   "="
        {			// > karakter
            kommentben = true;
            continue;
        }

        if (b == 0x0a) // \0x00   NULL    null 
        {			// újsor
            kommentben = false;
            continue;
        }
        ...
        if (b == 0x4e)		// N betű |78 hexben| \0x4e   N
            continue;
    ...
    kiFile << "0-ák száma = " << binFa.nullasz << std::endl;
    kiFile << "1-esek száma = " << binFa.egysza << std::endl;
